...
public class QLyKhachHangController {
    ...
    private void handleThoat() {
        try {
            ...
            e.printStackTrace();
            hienThongBao(Alert.AlertType.ERROR, "Lỗi hệ thống", "Không thể quay lại màn hình chính!");
        }
    }
    ...
    private void handleMoSuaKhachHang() {
        ...
        try {
            // Lưu khách hàng cần sửa để SuaKhachHangController lấy
            LuuThongTinDangNhap.khachHangDangSua = danhSachDuocChon.get(0);
            App.setRoot("SuaKhachHang");
        } catch (IOException e) {
            e.printStackTrace();
