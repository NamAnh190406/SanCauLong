...
private void handleMoSuaDichVu() {
    ...
    try {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("SuaDV.fxml"));
        Parent view = loader.load();
        SuaDichVuController controllerSua = loader.getController();
        controllerSua.setDichVuBanDau(dsChon.get(0));
        tableDichVu.getScene().setRoot(view);
    } catch (IOException e) {
        e.printStackTrace();
        hienThongBao(Alert.AlertType.ERROR, "Lỗi hệ thống", "Không tìm thấy tệp SuaDV.fxml!");
    }
}
...
private void handleMoThemDichVu() {
    try {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("ThemDV.fxml"));
        Parent view = loader.load();
        tableDichVu.getScene().setRoot(view);
    } catch (IOException e) {
        e.printStackTrace();
