...
private void handleLogin() {
    ...
    try {
        ...
        if (role.startsWith("KH")) {
            ...
            App.setRoot("ManHinhChinhQTV"); // ← tên file FXML của màn hình QTV
        } else {
            stackError.setVisible(true);
            stackError.setManaged(true);
        }
...
private String checkLogin() {
    ...
    String sql = "SELECT MA_TK FROM TAIKHOAN WHERE Username = ? AND Password = ?";
    ...
    try (Connection conn = DBContext.KetNoi();
        ...
        try (ResultSet rs = pstmt.executeQuery()) {
            if (rs.next()) {
                ...
                System.out.println(">>> MA_TK tim duoc: " + maTK);

                // Thử tìm trong KHACHHANG
                String sqlKH = "SELECT kh.MAKH, kh.HOTEN, kh.DIEMTICHLUY, " +
                               "kh.HANGTHANHVIEN, kh.SDT, kh.EMAIL, kh.NGAYDK " +
                               "FROM KHACHHANG kh WHERE kh.MA_TK = ?";
                try (PreparedStatement p2 = conn.prepareStatement(sqlKH)) {
                    ...
                    try (ResultSet rs2 = p2.executeQuery()) {
                        if (rs2.next()) {
                            ...
                            System.out.println(">>> Khach hang: " + LuuThongTinDangNhap.hoTen);
                            return LuuThongTinDangNhap.maKH; // "KH..."
                        }
                    }
                }

                // Thử tìm trong NHAN_VIEN
                String sqlNV = "SELECT MA_NV FROM NHAN_VIEN WHERE MA_TK = ?";
                try (PreparedStatement p3 = conn.prepareStatement(sqlNV)) {
                    p3.setString(1, maTK);
                    try (ResultSet rs3 = p3.executeQuery()) {
                        if (rs3.next()) {
                            String maNV = rs3.getString("MA_NV");
                ...
                // Không có trong KH lẫn NV → chắc chắn là Quản Trị Viên
                System.out.println(">>> Quan tri vien voi MA_TK: " + maTK);
                return "QTV_" + maTK; // bắt đầu bằng "QTV" → vào ManHinhChinhQTV
