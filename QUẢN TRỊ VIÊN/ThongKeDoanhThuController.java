...
public class ThongKeDoanhThuController {
    ...
    private void goHome() {
        try {
            Parent homeView = FXMLLoader.load(getClass().getResource("TrangChuAdmin.fxml"));
            if (TrangChuController.rootPane != null) {
                TrangChuController.rootPane.setCenter(homeView);
            }
        } catch (IOException e) {
            System.out.println("Lỗi chuyển màn hình về Trang Chủ!");
            e.printStackTrace();
        }
    }
}