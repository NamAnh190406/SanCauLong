...
public class SanController {
    ...
    private void handleXoaSan() {
        ...
        if (dsChon.isEmpty()) {
            hienThongBao(Alert.AlertType.WARNING, "Thông báo", "Vui lòng chọn ít nhất một sân bóng để xóa!");
            return;
        }
        ...
        if (res.isPresent() && res.get() == btnOk) {
            ...
            try (Connection conn = DBConnection.getConnection();
                ...
                pstmt.executeBatch();
                hienThongBao(Alert.AlertType.INFORMATION, "Thành công", "Đã xóa dữ liệu sân bóng thành công!");
                loadDataFromOracle();
            } catch (SQLException e) {
                hienThongBao(Alert.AlertType.ERROR, "Lỗi hệ thống", "Không thể xóa. Sân bóng có thể đã dính dữ liệu hóa đơn/lịch đặt: " + e.getMessage());
            }
        }
    }
    ...
    private void handleMoSuaSan() {
        ...
        if (dsChon.size() > 1) {
            hienThongBao(Alert.AlertType.WARNING, "Thông báo", "Hệ thống chỉ hỗ trợ sửa thông tin một sân bóng tại một thời điểm!");
            return;
        }
    ...
    private void handleThoat() {
        try {
            // Quay trở lại trang menu chọn trung gian Sân & Khung Giờ lúc nãy
